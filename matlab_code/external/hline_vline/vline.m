function [hhh,hLabelText]=vline(x,in1,in2)
...
if length(x)>1  % vector input
    for I=1:length(x)
        ...
        [h(I),hLabelText]=vline(x(I),linetype,label);
    end
else
    switch nargin
    case 1
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
           hLabelText = text(x+0.01*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'));
        else
           hLabelText = text(x-.05*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'));
        end
    end     
