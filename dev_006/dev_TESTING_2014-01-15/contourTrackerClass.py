		...
		if backgroundList == []:
			...
			for backgroundIndex, backgroundFile in enumerate(backgroundList):
				...
				if backgroundIndex == 0:
					backgroundDataSum = np.asarray(bkgrdata, dtype=np.float32).reshape(bkgr.size)
				else:
					backgroundDataSum = backgroundDataSum + np.asarray(bkgrdata, dtype=np.float32).reshape(bkgr.size)
...
#~ 
		...
		for strideNr in xrange(self.nrOfStrides):
			...
			self.host_maxFitIncline = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float64)
			self.dev_maxFitIncline = cl_array.to_device(self.queue, self.host_maxFitIncline)
		
			self.host_avgBkgrIntensity = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float64)
			self.dev_avgBkgrIntensity = cl_array.to_device(self.queue, self.host_avgBkgrIntensity)
			...
			self.prg.findMembranePositionNew2(self.queue, self.trackingGlobalSize, self.trackingWorkGroupSize, self.sampler, \
											 ...
											 self.dev_maxFitIncline.data, \
											 self.dev_avgBkgrIntensity.data \
											 )
			
			cl.enqueue_read_buffer(self.queue, self.dev_maxFitIncline.data, self.host_maxFitIncline).wait()
			cl.enqueue_read_buffer(self.queue, self.dev_avgBkgrIntensity.data, self.host_avgBkgrIntensity).wait()
			
			plt.plot(self.host_maxFitIncline), plt.show()
			plt.plot(self.host_avgBkgrIntensity), plt.show()
			ipdb.set_trace()
