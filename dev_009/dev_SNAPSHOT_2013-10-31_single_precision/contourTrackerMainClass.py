...
class contourTrackerMain( object ):
	def __init__(self, configurationFile):
		...
		self.contourCoordinatesX = np.empty((self.nrOfContourPoints,self.totalNrOfImages),dtype=np.float32)
		self.contourCoordinatesY = np.empty((self.nrOfContourPoints,self.totalNrOfImages),dtype=np.float32)
		
		self.contourNormalVectorsX = np.empty((self.nrOfContourPoints,self.totalNrOfImages),dtype=np.float32)
		self.contourNormalVectorsY = np.empty((self.nrOfContourPoints,self.totalNrOfImages),dtype=np.float32)
		
		self.contourCenterCoordinatesX = np.empty(self.totalNrOfImages,dtype=np.float32)
		self.contourCenterCoordinatesY = np.empty(self.totalNrOfImages,dtype=np.float32)
		
		self.nrOfIterationsPerContour = np.zeros(self.totalNrOfImages,dtype=np.int32)
		self.executionTimePerContour = np.zeros(self.totalNrOfImages,dtype=np.float32)
	...
	def setupClVariables(self):
		...
		self.host_mostRecentMembraneCoordinatesX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_mostRecentMembraneCoordinatesX = cl_array.to_device(self.managementQueue, self.host_mostRecentMembraneCoordinatesX)
		self.host_mostRecentMembraneCoordinatesY = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_mostRecentMembraneCoordinatesY = cl_array.to_device(self.managementQueue, self.host_mostRecentMembraneCoordinatesY)
		
		self.host_mostRecentMembraneNormalVectorsX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_mostRecentMembraneNormalVectorsY = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
			...
			for tracker in self.trackingQueues:
				...
				if tracker.iterationFinished:
					...
					if tracker.checkTrackingFinished():
						...
						self.executionTimePerContour[frameId] = np.float32(tracker.getExectionTime())
	...
	def setupClContext(self):
		...
		print "\tOpenCL Platform: "+self.clPlatformList[platformIndex].name
