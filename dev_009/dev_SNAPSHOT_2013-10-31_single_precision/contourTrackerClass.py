...
class contourTracker( object ):
	...
	def loadImage(self, imagePath):
		...
		self.meanIntensity = np.float32(np.mean(self.intensityRoiMatrix))
	...
	# json.loads(self.config.get("SectionOne","startingCoordinate"))
		...
		self.meanRangePositionOffset = np.float32(json.loads(config.get("TrackingParameters","meanRangePositionOffset")))
		
		self.localAngleRange = np.float32(json.loads(config.get("TrackingParameters","localAngleRange")))
		...
		self.coordinateTolerance = np.float32(json.loads(config.get("TrackingParameters","coordinateTolerance")))
		...
		self.inclineTolerance = np.float32(json.loads(config.get("TrackingParameters","inclineTolerance")))
		...
		self.centerTolerance = np.float32(json.loads(config.get("TrackingParameters","centerTolerance")))
		
		self.maxInterCoordinateAngle = np.float32(json.loads(config.get("TrackingParameters","maxInterCoordinateAngle")))
		self.maxCoordinateShift = np.float32(json.loads(config.get("TrackingParameters","maxCoordinateShift")))
		
		#~ self.minAngleDifference = np.float32(json.loads(config.get("TrackingParameters","minAngleDifference")))
	...
	def setupTrackingParameters(self):
		...
		self.linFitSearchRangeXvalues = np.float32(np.transpose(np.linspace(linFitSearchRangeStartXvalue,linFitSearchRangeEndXvalue,self.linFitSearchRangeNrOfInterpolationPoints)))
		...
		self.meanRangeXvalues = np.array(np.transpose(np.linspace(0,self.meanParameter,self.meanParameter)),dtype=np.float32)
		...
		localRotationMatrices = np.empty((localAngles.shape[0],2,2),np.dtype(np.float32))
		...
		#~ nrOfDetectionAngleSteps = np.float32(8000);
		#~ nrOfDetectionAngleSteps = np.float32(2000);
		#~ nrOfDetectionAngleSteps = np.float32(1365); # this is roughly, where we currently run out of local memory; when we still used it in some kernels
		self.nrOfDetectionAngleSteps = np.float32(self.nrOfStrides*self.detectionKernelStrideSize)
		
		self.angleStepSize = np.float32((detectionEndAngle-detectionStartAngle)/self.nrOfDetectionAngleSteps)
	...
	def setupClTrackingVariables(self):
		self.host_fitIncline = np.empty(self.nrOfLocalAngleSteps,dtype=np.float32)
		self.dev_fitIncline = cl_array.to_device(self.queue, self.host_fitIncline)
		
		self.host_fitIntercept = np.empty(self.nrOfLocalAngleSteps,dtype=np.float32)
		self.dev_fitIntercept = cl_array.to_device(self.queue, self.host_fitIntercept)
		
		self.host_localMembranePositionsX = np.zeros(shape=self.nrOfLocalAngleSteps,dtype=np.float32)
		#~ host_localMembranePositionsX = np.zeros(shape=self.localAngles.shape[0],dtype=np.float32)
		self.dev_localMembranePositionsX = cl_array.to_device(self.queue, self.host_localMembranePositionsX)

		self.host_localMembranePositionsY = np.zeros(shape=self.nrOfLocalAngleSteps,dtype=np.float32)
		#~ host_localMembranePositionsY = np.zeros(shape=localAngles.shape[0],dtype=np.float32)
		self.dev_localMembranePositionsY = cl_array.to_device(self.queue, self.host_localMembranePositionsY)

		self.host_membraneCoordinatesX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_membraneCoordinatesY = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_interCoordinateAngles = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_interpolatedMembraneCoordinatesX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_interpolatedMembraneCoordinatesX = cl_array.to_device(self.queue, self.host_interpolatedMembraneCoordinatesX)
		self.host_interpolatedMembraneCoordinatesY = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_interpolatedMembraneCoordinatesY = cl_array.to_device(self.queue, self.host_interpolatedMembraneCoordinatesY)

		self.host_previousInterpolatedMembraneCoordinatesX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_previousInterpolatedMembraneCoordinatesY = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_ds = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_ds = cl_array.to_device(self.queue, self.host_ds)

		self.host_sumds = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_sumds = cl_array.to_device(self.queue, self.host_sumds)
		
		self.host_membraneNormalVectorsX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_membraneNormalVectorsY = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_membranePolarRadius = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_interpolatedMembranePolarRadius = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_interpolatedMembranePolarRadius = cl_array.to_device(self.queue, self.host_interpolatedMembranePolarRadius)

		self.host_membranePolarTheta = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_angleDifferencesUpper = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_angleDifferencesUpper = cl_array.to_device(self.queue, self.host_angleDifferencesUpper)
		self.host_angleDifferencesLower = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_angleDifferencesLower = cl_array.to_device(self.queue, self.host_angleDifferencesLower)
				
		self.host_membranePolarRadiusInterpolation = np.zeros(shape=2*self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_membranePolarThetaInterpolation = np.zeros(shape=2*self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		#~ host_interpolationAngles = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		#~ host_interpolationAngles = angleStepSize*np.float32(range(self.nrOfDetectionAngleSteps))
		startAngle = -np.pi
		endAngle = np.pi - 2*np.pi/self.nrOfDetectionAngleSteps # we substract, so that we have no angle overlap
		#~ self.host_interpolationAngles = np.float32(np.linspace(-np.pi,np.pi,self.nrOfDetectionAngleSteps))
		self.host_interpolationAngles = np.float32(np.linspace(startAngle,endAngle,self.nrOfDetectionAngleSteps))
		self.dev_interpolationAngles = cl_array.to_device(self.queue, self.host_interpolationAngles)
		
		self.host_b = np.zeros(shape=2*self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_b = cl_array.to_device(self.queue, self.host_b)

		self.host_c = np.zeros(shape=2*self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_c = cl_array.to_device(self.queue, self.host_c)

		self.host_d = np.zeros(shape=2*self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_d = cl_array.to_device(self.queue, self.host_d)

		self.host_dbgOut = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.dev_dbgOut = cl_array.to_device(self.queue, self.host_dbgOut)
		
		self.host_dbgOut2 = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		...
		self.host_membraneNormalVectorsX = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		self.host_membraneNormalVectors = np.zeros(self.nrOfDetectionAngleSteps, cl.array.vec.float2)
		...
		#~ self.host_fitIncline = np.empty(self.nrOfLocalAngleSteps,dtype=np.float32)
		self.host_rotatedUnitVector = np.zeros(self.nrOfLocalAngleSteps, cl.array.vec.float2)
		...
		self.host_contourCenter = np.zeros(1, cl.array.vec.float2)
		...
		self.host_radialVectors = np.zeros(shape=self.nrOfDetectionAngleSteps, dtype=cl.array.vec.float2)
		
		#~ self.host_interpolationAngles = np.float32(np.linspace(startAngle,endAngle,self.nrOfDetectionAngleSteps))
		#~ self.dev_interpolationAngles = cl_array.to_device(self.queue, self.host_interpolationAngles)

		
		#~ angle = self.angleStepSize*np.float32(coordinateIndex+1)
		radiusUnitVector = np.array([1,0],dtype=np.float32)
		...
		for counter, angle in enumerate(self.host_interpolationAngles):
			...
			#~ ipdb.set_trace()
			self.host_radialVectors[counter] = radiusVectorRotationMatrix.dot(radiusUnitVector)
	...
	def trackContour(self):
		...
		#self.host_membraneCoordinatesXdebug = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
		#self.host_membraneCoordinatesYdebug = np.zeros(shape=self.nrOfDetectionAngleSteps,dtype=np.float32)
...
#~ 
		...
		#print self.getNrOfTrackingIterations()
			...
			##~ self.host_interpolationAnglesOld = np.float32(np.zeros(2048))
...
#~ 
		...
		#~ if self.getContourId() >= 0 and self.getNrOfTrackingIterations() >= 0:
			...
			#~ cl.enqueue_read_buffer(self.queue, self.dev_dbgOut.data, self.host_dbgOut).wait()
			#~ cl.enqueue_read_buffer(self.queue, self.dev_dbgOut2.data, self.host_dbgOut2).wait()
			#~ cl.enqueue_read_buffer(self.queue, self.dev_radialVectors.data, self.host_radialVectors).wait()
						
			#~ plt.imshow(self.host_Img)
			#~ ax = plt.gca()
			#~ plt.plot(self.host_membraneCoordinatesX,self.host_membraneCoordinatesY,'r')
			#~ plt.plot(self.host_interpolatedMembraneCoordinatesX,self.host_interpolatedMembraneCoordinatesY,'k'), plt.show()
			#~ ax.invert_yaxis() # needed so that usage of 'plt.quiver' (below), will play along
			#~ normalVectorScalingFactor = 3e-2
			#~ plt.quiver(self.host_membraneCoordinatesX,self.host_membraneCoordinatesY,self.host_membraneNormalVectorsX,self.host_membraneNormalVectorsY,units='dots', pivot='tail',scale=normalVectorScalingFactor) #, pivot='middle'
			
			#~ plt.plot(self.host_membranePolarTheta,self.host_membranePolarRadius,'k')
			#~ plt.show()		
			#~ ipdb.set_trace()
