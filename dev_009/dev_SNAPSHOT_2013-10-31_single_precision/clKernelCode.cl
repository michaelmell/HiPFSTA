...
void fit(__constant float x[], float y[], int gradientCenterIndex, int linFitParameter, float *a, float *b, float *siga, float *sigb, float *chi2)
{
	int i;
...
__kernel void findMembranePosition(sampler_t sampler, 
								   ...
								   const int imgSizeX,
								   const int imgSizeY,
								   __constant float* localRotationMatrices, // this should be local or constant
								   __constant float* linFitSearchRangeXvalues, // this should be local or constant
								   const int linFitParameter,
								   //~ __local float* fitIntercept,
								   //~ __local float* fitIncline,
								   __global float* fitIntercept,
								   __global float* fitIncline,
								   const int meanParameter,
								   __constant float* meanRangeXvalues, // this should be local or constant
								   const float meanRangePositionOffset,
								   //~ __local float* localMembranePositionsX,
								   //~ __local float* localMembranePositionsY,
								   __global float* localMembranePositionsX,
								   __global float* localMembranePositionsY,
								   __global float* membraneCoordinatesX,
								   __global float* membraneCoordinatesY,
								   __global float* membraneNormalVectorsX,
								   __global float* membraneNormalVectorsY,
								   const int coordinateIndex)
{
	...
	__private float lineIntensities[400];
	
	__private float2 membraneNormalVector;
	...
	__private float2 rotatedUnitVector;
	...
	__private float minValue = 32000; // initialize value with first value of array
	__private float maxValue = 0; // initialize value with first value of array
	
	__private float2 Coords;
	__private float2 NormCoords;
	...
	__private float2 basePoint = {membraneCoordinatesX[coordinateIndex],membraneCoordinatesY[coordinateIndex]};
	...
	__private float gradientCenterValue = minValue+(maxValue-minValue)/2.0;
	__private float minValue2 = 20000;
	__private float refValue;
	
	__private float a=0.0, b=0.0, siga=0.0, sigb=0.0, chi2=0.0;
	...
	if(minIndex<maxIndex){
		for(int index=minIndex;index<maxIndex;index++){
			...
			minValue2 = select(minValue2,refValue,(minValue2 > refValue));
		}
	...
	__private float meanIntensity = 0.0;
	...
	__private float relativeMembranePositionLocalCoordSys;
	//~ __private float membranePositionsX, membranePositionsY;
	...
	__private float xTmp = 0.0, yTmp = 0.0, meanIncline = 0.0;
	if(xInd==0){
...
__kernel void calculateMembraneNormalVectors(__global float* membraneCoordinatesX,
											 __global float* membraneCoordinatesY,
											 __global float* membraneNormalVectorsX,
											 __global float* membraneNormalVectorsY
											 //~ __local float2* membraneNormalVectors
										)
	{
		const int xInd = get_global_id(0);
		const int xSize = get_global_size(0);
		
		__private float vectorNorm;
...
__kernel void calculateContourCenter(__global float* membraneCoordinatesX,
									 __global float* membraneCoordinatesY,
									 __local float* ds,
									 __local float* sumds,
									 __global float2* contourCenter
									 //const float2 contourCenter
									 )
{
	const int xInd = get_global_id(0);
	const int xSize = get_global_size(0);
	...
	__private float circumference=0.0;
	...
	__private float tmp1=0.0, tmp2=0.0;
	if(xIndLoc==0){
		for(int index=0;index<xSize;index++){
...
__kernel void calculateContourCenterNew(__global float* membraneCoordinatesX,
									 __global float* membraneCoordinatesY,
									 //~ __local float* ds,
									 //~ __local float* sumds,
									 __global float* ds,
									 __global float* sumds,
									 __global float2* contourCenter
									 //~ const float2 contourCenter
									 )
{
...
__kernel void calculateDs(__global float* membraneCoordinatesX,
						 __global float* membraneCoordinatesY,
						 __global float* ds
						 )
{
...
__kernel void calculateSumDs(__global float* membraneCoordinatesX,
									 __global float* membraneCoordinatesY,
									 __global float* ds,
									 __global float* sumds
									 )
...
__kernel void calculateContourCenterNew2(__global float* membraneCoordinatesX,
										 __global float* membraneCoordinatesY,
										 __global float* ds,
										 __global float* sumds,
										 __global float2* contourCenter,
...
__kernel void cart2pol(__global float* membraneCoordinatesX,
					   __global float* membraneCoordinatesY,
					   __global float* membranePolarRadius,
					   __global float* membranePolarTheta,
					   __global float2* contourCenter
					  )
{
	const int xInd = get_global_id(0);
...
__kernel void pol2cart(__global float* membraneCoordinatesX,
					   __global float* membraneCoordinatesY,
					   __global float* membranePolarRadius,
					   __global float* membranePolarTheta,
					   __global float2* contourCenter
...
__kernel void emptyKernel(__global float* membraneCoordinatesX, __global float* membraneCoordinatesY)
{
	const int xInd = get_global_id(1);
	const int yInd = get_global_id(0);
	const int xSize = get_global_size(1);
...
//~ int spline (int n, int end1, int end2, float slope1, float slope2, float x[], float y[], float b[], float c, float d[], int *iflag)
...
int spline(const int n, int end1, int end2, float slope1, float slope2, __global float x[], __global float y[], __global float b[], __global float c[], __global float d[], int* iflag)
...
//~ float seval (n, u, x, y, b, c, d, last)
//~ int   n;
//~ float u;
//~ float x[], y[], b[], c[], d[];
//~ int    *last;

float seval(const int n, float u, __global float x[], __global float y[], __global float b[], __global float c[], __global float d[], int* last)
//~ __constant float 
...
{  /* begin function seval() */
	...
	*last = i;
	...
	//~ float t1=0.007, t2=0.003, result=0.0;
	//~ if(x[closestIndex]>u){
		//~ result = 1.0;
	//~ }
...
__kernel void interpolatePolarCoordinates(__global float* membranePolarRadius,
										  __global float* membranePolarTheta,
										  //~ __local float* membranePolarRadiusLoc,
										  //~ __local float* membranePolarThetaLoc,
										  __global float* membranePolarRadiusLoc,
										  __global float* membranePolarThetaLoc,
										  //~ __local float* interpolationMembranePolarRadius,
										  //~ __local float* interpolationMembranePolarTheta,
										  __global float* interpolationMembranePolarRadius,
										  __global float* interpolationMembranePolarTheta,
										  __global float* interpolationMembranePolarRadiusTesting,
										  __global float* interpolationMembranePolarThetaTesting,
										  __global float* interpolationAngles,
										  const int nrOfInterpolationPoints,
										  const int nrOfContourPoints,
										  __global float* dbgOut,
										  __global float* b,
										  __global float* c,
										  __global float* d
										 )
{
	...
	//~ __local float membranePolarThetaLoc[500], membranePolarRadiusLoc[500]; //, interpolationAnglesLoc[500];
	//~ __local float b[500], c[500], d[500];
	
	//~ __local int iflag;
	__private int iflag;
	//~ __local int last;
	
	//~ __private float thetaMaxVal = -10; // initial value larger 2*pi
	//~ __private float thetaMinVal = 10; // initial value smaller -2*pi
	...
	//if(xIndLoc==xIndRef){
		////~ printf("bla");
		//float test = seval(nrOfInterpolationPoints, 1.0, membranePolarThetaLoc, membranePolarRadiusLoc, b, c, d, &last);
		//printf("test : %f\n",test);
	...
	//~ int index = 248;
	//int index = 448;
	//float interpolationAngle = interpolationAngles[index];
	//float output;
	//if(xInd==0){
...
__kernel void checkIfTrackingFinished(
									__global float* interpolatedMembraneCoordinatesX,
									__global float* interpolatedMembraneCoordinatesY,
									__global float* previousInterpolatedMembraneCoordinatesX,
									__global float* previousInterpolatedMembraneCoordinatesY,
									__global int* trackingFinished,
									const float coordinateTolerance
...
__kernel void sortPolarCoordinates(__global float* membranePolarRadius,
								  __global float* membranePolarTheta,
								  //~ __local float* membranePolarRadiusLoc,
								  //~ __local float* membranePolarThetaLoc,
								  //~ __local float* membranePolarRadiusLoc,
		   ...
		   for(int i=1;i<=n-1;i++){
			  //~ if A[i-1] > A[i] then
				if(membranePolarTheta[i-1]>membranePolarTheta[i]){
				 //~ swap(A[i-1], A[i])
					__private float tmp1 = membranePolarTheta[i-1];
					membranePolarTheta[i-1] = membranePolarTheta[i];
					membranePolarTheta[i] = tmp1;
					__private float tmp2 = membranePolarRadius[i-1];
...
__kernel void sortCoordinates(__global float* membranePolarRadius,
							  __global float* membranePolarTheta,
							  __global float* membraneCoordinatesX,
							  __global float* membraneCoordinatesY,
							  __global float* membraneNormalVectorsX,
...
__kernel void findMembranePositionNew2(sampler_t sampler, 
									   ...
									   __local float* fitIntercept,
									   __local float* fitIncline,
									   __local float2* rotatedUnitVector2,
									   //~ __global float* fitIntercept,
									   //~ __global float* fitIncline,
									   const int meanParameter,
									   __constant float* meanRangeXvalues, // this should be local or constant
									   ...
									   const int inclineRefinementRange)
{
	...
	__private float a=0.0, b=0.0, siga=0.0, sigb=0.0, chi2=0.0;
	__private float aTmp=0.0, bTmp=0.0;
	...
	for(int index=imgSizeY/2-inclineRefinementRange;index<imgSizeY/2+inclineRefinementRange;index++){
		...
		__private float bTmp2 = bTmp;
		bTmp = select(bTmp,b,(b > bTmp2));
		aTmp = select(aTmp,a,(b > bTmp2));
	}
	//~ if(xInd==0 && yInd==0){
		//~ printf("aTmp: %f \n",aTmp);
		//~ printf("bTmp: %f \n",bTmp);
	 ...
	 * ****************************************************************/
	__local float maxFitIncline;
	//~ __local float minIncline;
	...
	//~ __private float inclineTolerance = 0.7; // TODO: this should be made an algorithm parameter in the settings file
	//~ __private float inclineTolerance = -0.9; // TODO: this should be made an algorithm parameter in the settings file
	...
	__private float xTmp = 0.0, yTmp = 0.0, inclineSum = 0.0;
	//~ __private float xMembraneNormalTmp = 0.0, yMembraneNormalTmp = 0.0, membraneNormalNorm, membraneNormalNormNew, membraneNormalNorm2;
	__private float xMembraneNormalTmp = 0.0, yMembraneNormalTmp = 0.0, membraneNormalNorm;
	//~ [xIndLoc+yIndLoc*xSizeLoc]
...
__kernel void filterNanValues(__global float* membraneCoordinatesX,
							  __global float* membraneCoordinatesY,
							  __global float* membraneNormalVectorsX,
							  __global float* membraneNormalVectorsY,
							  //~ __local int* closestLowerNoneNanIndexLoc,
							  //~ __local int* closestUpperNoneNanIndexLoc
							  __global int* closestLowerNoneNanIndexLoc,
							  __global int* closestUpperNoneNanIndexLoc
							  //~ __global float* dbgOut,
							  //~ __global float* dbgOut2
							 )
{
	const int xInd = get_global_id(0);
	const int xSize = get_global_size(0);
	...
	if(distToLowerIndex!=0 & distToUpperIndex!=0){
		...
		membraneCoordinatesX[xInd] = ((float)distToLowerIndex * membraneCoordinatesX[closestLowerNoneNanIndexLoc[xInd]] 
									+ (float)distToUpperIndex * membraneCoordinatesX[closestUpperNoneNanIndexLoc[xInd]])
									/(float)(distToLowerIndex+distToUpperIndex);
		...
		membraneNormalVectorsX[xInd] = ((float)distToLowerIndex * membraneNormalVectorsX[closestLowerNoneNanIndexLoc[xInd]] 
									  + (float)distToUpperIndex * membraneNormalVectorsX[closestUpperNoneNanIndexLoc[xInd]])
									  /(float)(distToLowerIndex+distToUpperIndex);
		...
		membraneNormalVectorsY[xInd] = ((float)distToLowerIndex * membraneNormalVectorsY[closestLowerNoneNanIndexLoc[xInd]] 
									  + (float)distToUpperIndex * membraneNormalVectorsY[closestUpperNoneNanIndexLoc[xInd]])
									  /(float)(distToLowerIndex+distToUpperIndex);

		//~ membraneNormalNorm = sqrt( pow(xMembraneNormalTmp,2) + pow(yMembraneNormalTmp,2) );
		
		float membraneNormalNorm;
...
//__kernel void filterLargeDsValues(__global float* membraneCoordinatesX,
								  //__global float* membraneCoordinatesY,
								  //__global float* membraneNormalVectorsX,
								  //__global float* membraneNormalVectorsY,
								  ...
								  //float tolerance, // distance an pixel that a point is allowed to be off (expected to be ~0.5...1 pixels)
								  //__global float* ds
								//)
//{
	//const int xInd = get_global_id(0);
...
__kernel void checkIfCenterConverged(
									__global float2* contourCenter,
									__global float2* previousContourCenter,
									__global int* trackingFinished,
									...
									) // will set self.dev_iterationFinished to true, when called
{
	...
	//~ __private float distance;
	__private float xDistance, yDistance;
	if(xInd==0){
...
__kernel void calculateInterCoordinateAngles(__global float* interCoordinateAngles,
											 __global float* membraneCoordinatesX,
											 __global float* membraneCoordinatesY
											 //~ __global float* dbgOut,
											 //~ __global float* dbgOut2
											 )
{
	...
	const int xSize = get_global_size(0);
	//~ __private float2 basePoint = {membraneCoordinatesX[coordinateIndex],membraneCoordinatesY[coordinateIndex]};
	__private float2 dsVector1, dsVector2;
	...
	//~ float dotProduct = dsVector1.x*dsVector2.x+dsVector1.y*dsVector2.y;
	float dotProduct = dot(dsVector1,dsVector2);
...
//~ int spline(const int n, int end1, int end2, float slope1,__global float c[], __global float d[]){
	void findClosestGoodCoordinates(const int xInd,
									...
									__global int* closestLowerCorrectIndexLoc,
									__global int* closestUpperCorrectIndexLoc,
									__global int* listOfGoodCoordinates,
									int* distToLowerIndex,
									int* distToUpperIndex
									)
	{
	//~ __private float distToLowerIndex = 0;
	//~ __private float distToUpperIndex = 0;
...
{
	if(*distToLowerIndex!=0 & *distToUpperIndex!=0){
		membraneCoordinatesX[xInd] = ((float)*distToLowerIndex * membraneCoordinatesX[closestLowerCorrectIndexLoc[xInd]] 
									+ (float)*distToUpperIndex * membraneCoordinatesX[closestUpperCorrectIndexLoc[xInd]])
									/(float)(*distToLowerIndex+*distToUpperIndex);
		membraneCoordinatesY[xInd] = ((float)*distToLowerIndex * membraneCoordinatesY[closestLowerCorrectIndexLoc[xInd]] 
									+ (float)*distToUpperIndex * membraneCoordinatesY[closestUpperCorrectIndexLoc[xInd]])
									/(float)(*distToLowerIndex+*distToUpperIndex);
...
__kernel void filterJumpedCoordinates(
											...
											__global int* closestUpperCorrectIndexLoc,
											__global int* listOfGoodCoordinates,
											const float maxCoordinateShift,
...
__kernel void filterIncorrectCoordinates(__global float2* previousContourCenter,
										 __global float* interCoordinateAngles,
										 __global float* membraneCoordinatesX,
										 __global float* membraneCoordinatesY,
										 __global float* membraneNormalVectorsX,
										 __global float* membraneNormalVectorsY,
										 ...
										 __global int* closestLowerCorrectIndexLoc,
										 __global int* closestUpperCorrectIndexLoc,
										 const float maxInterCoordinateAngle
										 //~ __global float* dbgOut,
										 //~ __global float* dbgOut2
										 )
{
...
__kernel void filterPolarCoordinateSingularities_OLD(__global float* membranePolarRadius,
												 __global float* membranePolarTheta,
												 const float minAngleDifference
												 //~ __global float* dbgOut,
												 //~ __global float* dbgOut2
												 )
{
	...
	//~ __private float minAngleDifference = 1e-3;
	...
	if(xInd>0 && xInd<xSize-3){ // calculate interior gradients
		if(fabs(membranePolarTheta[xInd] - membranePolarTheta[xInd+1]) < minAngleDifference){
			__private float meanAngle = (membranePolarTheta[xInd] + membranePolarTheta[xInd+1])/2;
			__private float meanRadius = (membranePolarRadius[xInd] + membranePolarRadius[xInd+1])/2;
		...
		if(fabs(membranePolarTheta[xInd] - membranePolarTheta[0]) < minAngleDifference){
			__private float meanAngle = (membranePolarTheta[xInd] + membranePolarTheta[0])/2;
			__private float meanRadius = (membranePolarRadius[xInd] + membranePolarRadius[0])/2;
...
__kernel void calculateAngleDifference( __global float* membranePolarTheta,
										__global float* angleDifferencesUpper,
										__global float* angleDifferencesLower
										)
{
	const int xInd = get_global_id(0);
	const int xSize = get_global_size(0);
...
__kernel void filterPolarCoordinateSingularities(__global float* membranePolarRadius,
												 __global float* membranePolarTheta,
												 __global float* angleDifferencesUpper,
												 __global float* angleDifferencesLower,
												 __global float2* previousContourCenter,
												 ...
												 )
{
	...
	//~ __private float angleDifference = fabs(membranePolarTheta[xInd] - membranePolarTheta[xInd+1]);
	...
	if(distToLowerIndex!=0 & distToUpperIndex!=0){
		membranePolarRadius[xInd] = ((float)distToLowerIndex * membranePolarRadius[closestLowerCorrectIndexLoc[xInd]] 
								   + (float)distToUpperIndex * membranePolarRadius[closestUpperCorrectIndexLoc[xInd]])
									/(float)(distToLowerIndex+distToUpperIndex);
		//~ membraneCoordinatesX[xInd] = ((float)distToLowerIndex * membraneCoordinatesX[closestLowerCorrectIndexLoc[xInd]] 
									//~ + (float)distToUpperIndex * membraneCoordinatesX[closestUpperCorrectIndexLoc[xInd]])
									//~ /(float)(distToLowerIndex+distToUpperIndex);
		
		membranePolarTheta[xInd] = ((float)distToLowerIndex * membranePolarTheta[closestLowerCorrectIndexLoc[xInd]] 
								  + (float)distToUpperIndex * membranePolarTheta[closestUpperCorrectIndexLoc[xInd]])
								   /(float)(distToLowerIndex+distToUpperIndex);
		//~ membraneCoordinatesY[xInd] = ((float)distToLowerIndex * membraneCoordinatesY[closestLowerCorrectIndexLoc[xInd]] 
									//~ + (float)distToUpperIndex * membraneCoordinatesY[closestUpperCorrectIndexLoc[xInd]])
									//~ /(float)(distToLowerIndex+distToUpperIndex);
...
__kernel void interpolatePolarCoordinatesLinear(__global float* membranePolarRadius,
											  __global float* membranePolarTheta,
											  __global float2* radialVectors,
											  __global float2* contourCenter,
											  __global float* membraneCoordinatesX,
											  __global float* membraneCoordinatesY,
											  __global float* interpolatedMembraneCoordinatesX,
											 ...
											 )
{
	...
	__private float2 radialLineBasePoint = contourCenter[0];
	__private float2 radialLineDirectionVector = radialVectors[xInd];
	
	__private float distanceFromCenter = 160000;
	__private float2 interpolatedMembranePoint;
	__private float2 radialVector;
	
	__private float interpolationAngle = interpolationAngles[xInd];
	for(index=xInd-nrOfAnglesToCompare/2+1;index<xInd+nrOfAnglesToCompare/2;index++)
	{
		...
		__private float lowerAngle = membranePolarTheta[lowerIndex];
		__private float upperAngle = membranePolarTheta[upperIndex];
		//~ __private float interpolationAngle = interpolationAngles[xInd];
		...
		if( interpolationAngle >= lowerAngle && interpolationAngle < upperAngle )
		{
			...
			__private float2 lineSegmentDirectionVector;
			__private float2 lineSegmentBasePoint;
			...
			//~ dbgOut[xInd] = m;
			//~ dbgOut2[xInd] = ( radialLineDirectionVector.x*lineSegmentDirectionVector.y
			       //~ -radialLineDirectionVector.y*lineSegmentDirectionVector.x );
			dbgOut[xInd] = radialLineDirectionVector.x;
			dbgOut2[xInd] = lineSegmentDirectionVector.y;
			//~ dbgOut[xInd] = radialLineDirectionVector.x*lineSegmentDirectionVector.y;
			//~ dbgOut2[xInd] = radialLineDirectionVector.y*lineSegmentDirectionVector.x;
			
			__private float2 interpolatedMembranePointTMP;
