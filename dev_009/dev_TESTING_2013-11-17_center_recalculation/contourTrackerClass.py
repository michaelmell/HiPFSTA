...
class contourTracker( object ):
	...
	# json.loads(self.config.get("SectionOne","startingCoordinate"))
		...
		self.maxCoordinateShift = np.float64(json.loads(config.get("TrackingParameters","maxCoordinateShift")))
...
#~ 
		...
		self.prg.interpolatePolarCoordinatesLinear(self.queue, self.gradientGlobalSize, None, \
		...
		#if self.getContourId() <= 10:
			#if self.getNrOfTrackingIterations() <= 1:
				## initialize arrays
				#self.centers = [[np.nan,np.nan]]
				#self.centersInterpolated = [[np.nan,np.nan]]
				#self.trackedCenter = [[np.nan,np.nan]]
				#self.meanRadii = [np.nan]
				
			#if self.getNrOfTrackingIterations() <= 299:
				#cl.enqueue_read_buffer(self.queue, self.dev_membraneCoordinatesX.data, self.host_membraneCoordinatesX).wait()
				#cl.enqueue_read_buffer(self.queue, self.dev_membraneCoordinatesY.data, self.host_membraneCoordinatesY).wait()
				...
				#newTrackedCenter = [[self.host_contourCenter[0]['x'],self.host_contourCenter[0]['y']]]
				#newCenter = [[np.mean(self.host_membraneCoordinatesX),np.mean(self.host_membraneCoordinatesY)]]
				#newCenterInterpolated = [[np.mean(self.host_interpolatedMembraneCoordinatesX),np.mean(self.host_interpolatedMembraneCoordinatesY)]]

				#self.centers = np.concatenate((self.centers, newCenter), axis=0)
				#self.centersInterpolated = np.concatenate((self.centersInterpolated, newCenterInterpolated), axis=0)
				#self.trackedCenter = np.concatenate((self.trackedCenter, newTrackedCenter), axis=0)
				
				#meanRadius = [np.mean(self.host_membranePolarRadius)]
				#self.meanRadii = np.concatenate((self.meanRadii, meanRadius), axis=0)
				
			#else:
				##~ print '[',np.mean(self.host_membraneCoordinatesX),',',np.mean(self.host_membraneCoordinatesY),']'
				##~ print '[',np.mean(self.host_interpolatedMembraneCoordinatesX),',',np.mean(self.host_interpolatedMembraneCoordinatesY),']'

				#self.centers = np.delete(self.centers, [0,1], 0)
				#self.centersInterpolated = np.delete(self.centersInterpolated, [0,1], 0)
				#self.trackedCenter = np.delete(self.trackedCenter, [0,1], 0)
				#self.meanRadii = np.delete(self.meanRadii, [0,1], 0)

				#ipdb.set_trace()
				
				##~ plt.plot(self.centers[:,0],self.centers[:,1],'r')
				##~ plt.plot(self.centersInterpolated[:,0],self.centersInterpolated[:,1])
				##~ plt.plot(self.trackedCenter[:,0],self.trackedCenter[:,1],'c')
				##~ plt.show()
				
				
				#plt.plot(self.meanRadii-np.mean(self.meanRadii),'c')
				#plt.plot(self.trackedCenter[:,1]-np.mean(self.trackedCenter[:,1]),'k')
				#plt.plot(self.trackedCenter[:,0]-np.mean(self.trackedCenter[:,0]),'r')
				#plt.show()
				
				##~ plt.plot(self.host_membranePolarRadius,'c')
				##~ plt.show()
			...
			#~ plt.show()
			#~ 
			#~ diffX = np.abs(np.diff(self.host_membraneCoordinatesX))
			#~ diffXinterp = np.abs(np.diff(self.host_interpolatedMembraneCoordinatesX))
			#~ nrOfHistBins = 30
			#~ hist, binEdges = np.histogram(diffX, density=True, bins=nrOfHistBins)
			#~ histInterp, binEdgesInterp = np.histogram(diffXinterp, density=True, bins=nrOfHistBins)
			#~ plt.plot(binEdges[:-1],hist)
			#~ plt.plot(binEdgesInterp[:-1],histInterp,'r')
			#~ plt.show()
#~ 
			#~ plt.plot(self.host_interpolationAngles)
			#~ plt.plot(self.host_dbgOut)
