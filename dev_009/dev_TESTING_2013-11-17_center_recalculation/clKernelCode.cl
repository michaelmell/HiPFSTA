									 ...
									 )
{
	...
	if(xInd==0){
		...
		//~ for(int index=0;index<nrOfContourPoints;index++){
			//~ tmp1 = tmp1 + membraneCoordinatesX[index];
			//~ tmp2 = tmp2 + membraneCoordinatesY[index];
		//~ }
		//~ contourCenter[0].x = tmp1/nrOfContourPoints;
		//~ contourCenter[0].y = tmp2/nrOfContourPoints;
									   ...
									   const int inclineRefinementRange)
{
	...
	for(int index=0;index<imgSizeY;index++){
		...
		//~ fNormCoords.x = (float)NormCoords.x;
		//~ fNormCoords.y = (float)NormCoords.y;
		//~ fNormCoords.x = convert_float(NormCoords.x);
		//~ fNormCoords.y = convert_float(NormCoords.y);
		fNormCoords = convert_float2(NormCoords);
		
		lineIntensities[index] = convert_double(read_imagef(Img, sampler, fNormCoords).x);
		//~ interpolatedIntensities[xInd+index*imgSizeX] = read_imagef(Img, sampler, NormCoords).x;
	...
	for(int index=0;index<meanParameter;index++){
		...
		//~ meanIntensity = meanIntensity + read_imagef(Img, sampler, fNormCoords).x;
		meanIntensity = meanIntensity + convert_double(read_imagef(Img, sampler, fNormCoords).x);
	}
	meanIntensity = meanIntensity/convert_float(meanParameter);
